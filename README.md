...
## 기수정렬
값을 비교하지 않는 정렬이다. 값을 놓고 비교할 자릿수를 정한 다음에 해당 자릿수만 비교한다.  
  
### 과정
기수 정렬은 10개의 큐를 이용한다.  
원본 배열 16, 80, 18, 77, 3, 24, 88, 23이 있을때 일의 자릿수 기준으로 배열 원소를 큐에 집어 넣는다.  
다음 0번째 큐부터 9번째 큐가지 pop을 진행한다. 그 결과 80,3, 23, 24, 16, 77, 18, 88이 만들어진다. 이어서 십의 자릿수를 기준으로 같은 과정을 진행한다.  
